...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
 ...
 //die();
        ...
        $e2t  = $e2w;
        $mt   = $mw + $mm;
        $st   = intval($sw) + intval($sm) + intval($sp);
        $agt  = intval($agw) + intval($agm);
        $bit  = intval($biw) + intval($bim);
        $rt   = intval($rw) + intval($rm);
        $it   = intval($iw) + intval($im) + intval($ip);
        $dt   = intval($dw) + intval($dm) + intval($dp);
        $spt  = intval($spw) + intval($spm) + intval($spp);
        $wlt  = intval($wlw) + intval($wlm) + intval($wlp);
        $at   = intval($aw) + intval($am) + intval($ap);

        $totalmark = $b1t + $b2t + $e1w + $e2w + $mt + $st + $agt +$bit  +$rt  +$it;
    ...
    //Bangla 1st paper Grade set...
        ...
        $totalgpa = $b1g + $b2g + $e1g + $e2g + $mg + $sg + $agg + $big + $rg + $ig ;

        $gpa = $totalgpa / $subNum;
        //$gpa = number_format($gpa, 2);
        $gpa = number_format($gpa, 3);

         echo $gpa." totalgpa ". $totalgpa." Subject: ". $subNum." totalmark: ".$totalmark;
        //die();


$stmt = $db->prepare("insert into sixtoeightmarksheet(class, shift, branch, term, year, student_id, bangla1, fullmark, b1w, b1m, b1t, b1g, bangla2, b2fullmark, b2w, b2m, b2t, b2g, english1, enfullmark, e1w, e1g, english2, e2fullmark, e2w, e2g, math, mfullmark, mw, mm, mt, mg, science, sfullmark, sw, sm, st, sg, agriculture, agfullmark, agw, agm,  agt, agg, banint, bifullmark, biw, bim, bit, big, religion, refullmark, rw, rm, rt, rg, ict, ictfullmark, iw, im, ip, it, ig, totalSub,  totalmark, gpa, created_at, unique_id )

             values( :class, :shift, :branch, :term, :year, :student_id, :bangla1, :fullmark, :b1w, :b1m, :b1t, :b1g, :bangla2, :b2fullmark, :b2w, :b2m, :b2t, :b2g, :english1, :enfullmark, :e1w, :e1g, :english2, :e2fullmark, :e2w, :e2g, :math, :mfullmark, :mw, :mm, :mt, :mg, :science, :sfullmark, :sw, :sm, :st, :sg, :agriculture, :agfullmark, :agw, :agm,   :agt, :agg, :banint, :bifullmark, :biw, :bim, :bit, :big, :religion, :refullmark, :rw, :rm, :rt, :rg, :ict, :ictfullmark, :iw, :im, :ip, :it, :ig, :totalSub,  :totalmark, :gpa, NOW(), :unique_id )  ");
...
// ));
            ...
            if($result){
                $_SESSION['msg'] = "All Subject Number Added Successfully ";
