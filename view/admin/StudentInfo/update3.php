...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    //student, father and mother image uploaded in the same time...

    if ( !empty($_FILES['student_image']['name']) && !empty($_FILES['father_image']['name']) && !empty($_FILES['mother_image']['name']) ){
        ...
        $_SESSION['imgSize'] = $imgSize;
             echo "<script>window.location= 'editStudent.php?edit=".$unique_id."'</script>";
    ...
    elseif (in_array($file_ext, $permited) == false) {
        ...
        if (!empty($_FILES['mother_image'])) {
            $mother_image = $_FILES['mother_image']['name'];
            $main_image_size = $_FILES['mother_image']['size'];
            $main_image_tmp = $_FILES['mother_image']['tmp_name'];

            $name = substr(md5('yuighkj'), '0', '10');
    ...
    elseif (in_array($file_ext, $permited) == false) {
        ...
        var_dump($_FILES);
        echo $unique_id.'<br>';
        echo $student_image." ".$father_image." ".$mother_image;
        //die();
       ...
       $stmt = $db->prepare("UPDATE `student_data` SET
        ...
        if($result){
         $_SESSION['msg'] = 'Data successfully Updated wwwwwwwwwww!!!';
    ...
    //student and father image uploaded in the same time...

    elseif ( !empty($_FILES['student_image']['name']) && !empty($_FILES['father_image']['name']) ){
 ...
 WHERE unique_id = :unique_id;");
    ...
    elseif ( !empty($_FILES['student_image']['name'])  && !empty($_FILES['mother_image']['name']) ){
    ...
    elseif (in_array($file_ext, $permited) == false) {
        ...
        if (!empty($_FILES['mother_image'])) {
            ...
            $name = substr(md5('oewruorue'), '0', '10');
            $data = explode('.', $mother_image);
            $mother_image = $num2 . '.' . end($data);
 ...
 WHERE unique_id = :unique_id;");
    ...
    //father and mother image same time uploaded...

    elseif (!empty($_FILES['father_image']['name']) && !empty($_FILES['mother_image']['name'])){
 ...
 WHERE unique_id = :unique_id;");
    ...
    //only student image upload here...

    elseif(!empty($_FILES['student_image']['name'])){
