...
class Ninetenscience extends Connection
{
	...
	public function nineTenAllMark($class, $term, $year, $student_id){
		try {
			...
			$stmt = $this->con->prepare(" select * from nineten where class=:class and term=:term and year=:year and student_id=:student_id and status=1 ");
	...
	//all gpa select 
	 public function selectNineTenMark($class, $shift, $branch, $department, $term, $year){
	 	try {
	 		...
	 		$stmt = $this->con->prepare("select * from nineten where class=:class and shift=:shift and branch=:branch and department=:department and term=:term and year=:year and status=1 ");
	 		$stmt->bindValue(':class', $class, PDO::PARAM_STR);
	 		...
	 		if($stmt->rowCount() > 0){
	 			...
	 			echo "<script>window.location='view/admin/ninetenmarksheet/science/meritmarksheet.php'</script>";
	 		}
	...
	public function studentNumberSelect($student_id){
		try {
			...
			$stmt = $this->con->prepare("select * from nineten where student_id=:student_id and status=1 ");
			$stmt->bindValue(':student_id', $student_id, PDO::PARAM_STR);
			$stmt->execute();
			if($stmt->rowCount() > 0){
				return $stmt->fetch(PDO::FETCH_ASSOC);
			}else{
				$_SESSION['delete'] = "Student ID Does or Other Value not match, Please try again !!!";
				echo "<script>window.location='view/admin/ninetenmarksheet/science/meritmarksheet.php'</script>";
	...
	//select gpa and total number for create actual merit position...
	 public function selectSingleGpaTotal($class, $term,  $year, $student_id){
	 	try {
	 		...
	 		$stmt = $this->con->prepare("select totalmark + gpa from nineten where class=:class and term=:term and year=:year and student_id=:student_id and status=1 ");
	 ...
	 //delete student marksheet data from database...
	 public function deleteNineTenMark($class, $term, $year, $student_id){
	 	try {

	 		$stmt = $this->con->prepare(' update nineten set status = :status, updated_at=NOW() where class=:class and term=:term and year=:year and student_id=:student_id ');
	 ...
	 public function selectNineTenMarkCommerce($class, $shift, $branch, $department, $term, $year){
	 	try {
	 		...
	 		if($stmt->rowCount() > 0){
	 			...
	 			$_SESSION['onefiveErr'] = "Value does not match !!!";
	 			echo "<script>window.location='view/admin/ninetenmarksheet/commerce/meritmarksheet.php'</script>";
	 		}
	...
	public function studentNumberSelectCommerce($class, $term, $year, $department, $student_id){
		try {
			...
			$stmt = $this->con->prepare("select * from nineten where class=:class and term=:term and year=:year and department=:department and  student_id=:student_id and status=1 ");
			$stmt->bindValue(':class', $class, PDO::PARAM_STR);
			$stmt->bindValue(':term', $term, PDO::PARAM_STR);
			$stmt->bindValue(':year', $year, PDO::PARAM_STR);
			$stmt->bindValue(':department', $department, PDO::PARAM_STR);
	...
	public function deleteNineTenMarkCommerce($class, $term, $year, $student_id){
		try {
			...
			$stmt = $this->con->prepare('update nineten set status=:status, deleted_at=NOW() where class=:class and term=:term and year=:year and student_id=:student_id ');
			...
			if($stmt){
				...
				echo "<script>window.location='view/admin/ninetenmarksheet/commerce/deleteStuMark.php'</script>";
			}
	 ...
	 public function selectNineTenMarkHumanities($class, $shift, $branch, $department, $term, $year){
	 	try {
	 		...
	 		if($stmt->rowCount() > 0){
	 			...
	 			$_SESSION['onefiveErr'] = "Value does not match !!!";
	 			echo "<script>window.location='view/admin/ninetenmarksheet/humanities/meritmarksheet.php'</script>";
	 		}
	...
	public function deleteNineTenMarkHumanities($class, $term, $year, $student_id){
		try {
			...
			if($stmt){
				...
				$_SESSION['deleteErr'] = "Student ID or Other field value does not match !!!";
				echo "<script>window.location='view/admin/ninetenmarksheet/humanities/deleteStuMark.php'</script>";
			}
	...
	// restore ssc student marksheet all department
	public function restoreSSCMarksheet($class, $dept, $term, $year, $student_id){
		try {

			$stmt = $this->con->prepare('update nineten set status=:status where class=:class and department=:dept and term=:term and year=:year and student_id=:student_id ');
			...
			if($stmt){
				...
				echo "<script>window.location='allDeleteSSCmarksheet.php'</script>";
			}
			else{
				$_SESSION['error'] = "Value does not match !!!";
				echo "<script>window.location='allDeleteSSCmarksheet.php'</script>";
			}
	...
	// select all student mark in one page...
	public function allmarksheet($class, $term, $year)
	{
		try {

			$stmt = $this->con->prepare("select * from nineten where class=:class and term=:term and year=:year and status=1 order by gpa desc");
			$stmt->bindValue(':class', $class, PDO::PARAM_STR);
			$stmt->bindValue(':term', $term, PDO::PARAM_STR);
	 		$stmt->bindValue(':year', $year, PDO::PARAM_STR);
	 		$stmt->execute();
