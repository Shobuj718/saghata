...
class Onetofive extends Connection
{
	...
	// delete student marksheet class one to five any student 
	public function deleteOneToFive($class, $year, $student_id, $term, $unique_id){
		try {

			$stmt = $this->con->prepare('select * from onetofivemark where class=:class and year=:year and student_id=:student_id and unique_id=:unique_id and status=1 ');
			$stmt->bindValue(':class', $class, PDO::PARAM_STR);
			$stmt->bindValue(':year', $year, PDO::PARAM_STR);
			$stmt->bindValue(':student_id', $student_id, PDO::PARAM_STR);
			$stmt->bindValue(':term', $term, PDO::PARAM_STR);
