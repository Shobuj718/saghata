...
class Gradeset extends Connection 
{
	...
	// select student gpa and total number from allmarks table...
	public function playToFiveStuGpa($class, $year){
		try {

			$stmt = $this->con->prepare("select student_id,totalmark,gpa from onetofivemark  where class=:class and year=:year and status=1 order by gpa desc");
	...
	// select six and eight gpa and all info from onetofivemarksheet
	public function sixToEightGpaNum($class, $year){
		try {
	...
	//select ssc student gpa and total number 
	public function sscStuGpaNum($class, $year){
		try {

			$stmt = $this->con->prepare("select student_id,totalmark,gpa from sciencemarksheet where class=:class and year=:year and status=1 order by gpa desc ");
			$stmt->bindValue(':class',$class, PDO::PARAM_STR);
			$stmt->bindValue(':year', $year, PDO::PARAM_STR);
			$stmt->execute();
			if($stmt){
	...
	// select hsc student totalmark and gpa
	public function hscStuGpaNum($class, $year){
		try {

			$stmt = $this->con->prepare("select student_id,totalmark,gpa from hsctable where class=:class and year=:year and status=1 order by gpa desc ");
			$stmt->bindValue(':class', $class, PDO::PARAM_STR);
			$stmt->bindValue(':year', $year, PDO::PARAM_STR);
			$stmt->execute();
			if($stmt){
				...
				$_SESSION['gradeseterr'] = "Class or Year does not match ";
				echo "<script>window.location='allStuGradeNum.php'</script>";
			}
	...
	// select other student totalmark and gpa
	public function stuGpaNum($class, $year){
		try {

			$stmt = $this->con->prepare("select student_id,totalmark,gpa from otherTable where class=:class and year=:year and status=1 ");
			$stmt->bindValue(':class', $class, PDO::PARAM_STR);
			$stmt->bindValue(':year', $year, PDO::PARAM_STR);
			$stmt->execute();
			if($stmt){
