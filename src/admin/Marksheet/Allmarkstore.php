...
class Allmarkstore extends Connection
{
 ...
 // 	private $student_id;
  ...
  //       }
	 ...
	 public function selectstudetn(){
	 	try {
	 		if(date('Y') == '2019'){
	 			//die();
	 			//echo 'alert("Expired ! Please Contact your services company !!!")';
	 			
	 			echo "<script>alert('select');
	 ...
	 public function select($id){
	 	try {
	 		...
	 		$stmt = $this->con->prepare("select * from sixtoeightmarksheet where student_id = :id ");
	 ...
	 //all gpa select 
	 public function selectAllgpa($class, $shift, $branch, $term, $year ){
	 	try {

	 		//$stmt = $this->con->prepare("select * from allmarks where status = 1 ");
	 		$stmt = $this->con->prepare("select * from sixtoeightmarksheet where class=:class and shift=:shift and branch=:branch and term=:term and year=:year and status=1 ");
 ...
 //select student total number...
	 public function studentNumberSelect2($student_id,$term){
	 	try {

	 		$stmt = $this->con->prepare("select * from sixtoeightmarksheet where student_id = :student_id and term=:term and status = 1 ");
	 		$stmt->bindValue(':student_id', $student_id, PDO::PARAM_STR);
	 		$stmt->bindValue(':term', $term, PDO::PARAM_STR);
	 		$stmt->execute();
	 ...
	 public function selectStudentTotalmark($student_id){
	 	try {
	 		...
	 		$stmt = $this->con->prepare("select totalmark from sixtoeightmarksheet where student_id=:student_id and status=1 ");
	 		$stmt->bindValue(':student_id', $student_id, PDO::PARAM_STR);
	 		$stmt->execute();
	 		return $stmt->fetch(PDO::FETCH_ASSOC);
	 ...
	 // student marksheet edit
	 public function editStuMark($class, $term ,  $year,  $student_id){
	 	try {

	 		$stmt = $this->con->prepare("select * from sixtoeightmarksheet where class=:class and term=:term and year=:year and student_id=:student_id and status=1 ");
	 		$stmt->bindValue(':class', $class, PDO::PARAM_STR);
	 		$stmt->bindValue(':term', $term, PDO::PARAM_STR);
	 		$stmt->bindValue(':year', $year, PDO::PARAM_STR);
	 		$stmt->bindValue(':student_id', $student_id, PDO::PARAM_STR);
	 		...
	 		if($stmt->rowCount()){
	 			...
	 			$_SESSION['markerr'] = "Student ID or other field does not match !!!";
	 			echo "<script>window.location='view/admin/marksheet/editallSubMarkSearch.php'</script>";
	 		}
	...
	//delete student marksheet data from database...
	 public function deleteStuMarksheet($class, $term, $year, $student_id){
	 	try {

	 		$stmt = $this->con->prepare(' select * from sixtoeightmarksheet where class=:class and term=:term and year=:year and student_id=:student_id and status=2');
	 		$stmt->bindValue(':class', $class, PDO::PARAM_STR);
	 		$stmt->bindValue(':term', $term, PDO::PARAM_STR);
	 		$stmt->bindValue(':year', $year, PDO::PARAM_STR);
	 		$stmt->bindValue(':student_id', $student_id, PDO::PARAM_STR);
	 ...
	 // delete marksheet 
	 public function deleteMarksheet($unique_id){
	 	try {

	 		$stmt = $this->con->prepare("update sixtoeightmarksheet set status = :status where unique_id = :id ");;
			$stmt->bindValue(':status', 1, PDO::PARAM_STR);
			$stmt->bindValue(':id', $unique_id, PDO::PARAM_STR);
			$stmt->execute();
	 		if($stmt){
	 			$_SESSION['deleteMarsheet2'] = "Marksheetwww Deleted Successfully";
	 			//echo "<script>window.location='deleteStuMark.php</script>";
	 ...
	 // select gpaTotalnum from allmark table...
	 public function gpaTotalnum($class, $term, $year, $student_id){
	 	try {

	 		$stmt = $this->con->prepare("select * from sixtoeightmarksheet where class=:class and term=:term and year=:year and student_id=:student_id ");
	 		$stmt->bindValue(':class', $class, PDO::PARAM_STR);
	 		$stmt->bindValue(':term', $term, PDO::PARAM_STR);
	 		$stmt->bindValue(':year', $year, PDO::PARAM_STR);
	 		$stmt->bindValue('student_id', $student_id, PDO::PARAM_STR);
	...
	//all gpa select 
	 public function Allgpa($class, $shift, $branch, $department, $term, $year ){
	 	try {

	 		//$stmt = $this->con->prepare("select * from allmarks where status = 1 ");
	 		$stmt = $this->con->prepare("select * from sixtoeightmarksheet where class=:class and shift=:shift and branch=:branch and department=:department and term=:term and year=:year and status=1 ");
	 		$stmt->bindValue(':class', $class, PDO::PARAM_STR);
	 		$stmt->bindValue(':shift', $shift, PDO::PARAM_STR);
	 		$stmt->bindValue(':branch', $branch, PDO::PARAM_STR);
